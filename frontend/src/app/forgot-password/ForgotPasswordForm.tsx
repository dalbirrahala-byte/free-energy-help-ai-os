...
export function ForgotPasswordForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!isCanonicalAuthOrigin(window.location.origin)) {
      setPending(false);
      setWrongOrigin(true);
      setError("Password recovery is blocked on preview or alternate hosts. Use the canonical FEH CRM recovery page.");
      return;
    }
  ...
  return (
    <form onSubmit={submit} className="mt-6 space-y-4">
      <div>
        ...
        <input id="email" name="email" type="email" required autoComplete="email" autoCapitalize="none" spellCheck={false} className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none" />
      </div>
      {error && <p role="alert" className="rounded-xl bg-red-50 px-4 py-2.5 text-sm font-medium text-red-700">{error}</p>}
      {wrongOrigin && (
        <a href={canonicalAuthUrl("/forgot-password")} className="block rounded-xl border border-emerald-200 px-4 py-2.5 text-center text-sm font-semibold text-emerald-700 hover:bg-emerald-50">
          Open canonical FEH CRM recovery
        </a>
      )}
